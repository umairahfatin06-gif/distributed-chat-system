// ============================================
// Server 2 - TechCom Nexus Chat System
// Port: 4000
...
io.on('connection', (socket) => {
    console.log(`[Server2:${PORT}] ✅ Client connected: ${socket.id}`);

    // Send current user list to the newly connected client
    redisUserManager.getUsers()
        .then(users => {
            console.log(`[Server2:${PORT}] Sending user list to new client:`, users);
            socket.emit('userList', users);
        })
        .catch(err => {
            console.error(`[Server2:${PORT}] Error getting users:`, err);
    ...
    socket.on('join', async (username) => {
        console.log(`[Server2:${PORT}] 📥 Join event received: "${username}" for socket ${socket.id}`);
        
        try {
            if (!username || username.trim() === '') {
                console.log(`[Server2:${PORT}] ❌ Empty username rejected`);
                return;
            }
            ...
            if (socket.username && socket.username !== username) {
                ...
                console.log(`[Server2:${PORT}] Socket changing username from "${oldUsername}" to "${username}"`);
            ...
            console.log(`[Server2:${PORT}] ✅ User added to Redis. Current users:`, allUsers);
            ...
            console.error(`[Server2:${PORT}] ❌ Error in join handler:`, error);
        }
    });
    ...
    socket.on('leave', async (username) => {
        try {
            if (username) {
                console.log(`[Server2:${PORT}] 📤 Leave event: ${username} leaving gracefully`);
                ...
                console.log(`[Server2:${PORT}] ${username} left. Current users:`, allUsers);
            }
        } catch (error) {
            console.error(`[Server2:${PORT}] ❌ Error in leave handler:`, error);
        }
    });
    ...
    socket.on('chatMessage', (data) => {
        console.log(`[Server2:${PORT}] 💬 Message from ${data.username}: ${data.text}`);
    ...
    socket.on('disconnect', async () => {
        try {
            if (socket.username) {
                ...
                console.log(`[Server2:${PORT}] 🔴 Client disconnected: ${disconnectedUser} (socket: ${socket.id})`);
                ...
                console.log(`[Server2:${PORT}] Current users after disconnect:`, allUsers);
            } else {
                console.log(`[Server2:${PORT}] 🔴 Unknown client disconnected: ${socket.id}`);
            }
        } catch (error) {
            console.error(`[Server2:${PORT}] ❌ Error in disconnect handler:`, error);
        }
    });
});
...
server.on('error', (error) => {
    console.error(`[Server2:${PORT}] ❌ Server error:`, error);
});
...
process.on('SIGINT', async () => {
    ...
    // Close server
    server.close(() => {
        console.log(`[Server2:${PORT}] ✅ Server closed`);
        process.exit(0);
    });
});